...
<!-- Hero Section -->
<section bind:this={sectionElement} class="relative min-h-screen overflow-hidden hero-section bg-white" style="background: radial-gradient(ellipse 60% 40% at 50% 40%, #dbeafe 0%, #e0e7ef 60%, #fff 100%);">
  <div class="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-10 md:pt-8 md:pb-12 lg:pb-10 lg:pt-12">
    <div class="flex flex-col-reverse md:flex-row md:items-center md:justify-between">
      ...
      >
        ...
        <div class="mt-8 flex flex-col sm:flex-row gap-4">
          <Button 
            size="lg" 
            class="rounded-full px-6 bg-blue-600 hover:bg-blue-700 text-white shadow transition-all duration-200 focus:ring-2 focus:ring-blue-300 focus:outline-none text-base"
          ...
          <Button 
            variant="outline" 
            size="lg" 
            class="rounded-full px-6 border-2 border-blue-600 text-blue-600 hover:bg-blue-50 shadow transition-all duration-200 focus:ring-2 focus:ring-blue-200 focus:outline-none text-base"
            on:click={() => {
      ...
      >
        <!-- Imagen en desktop -->
        <div 
          class="hidden md:block absolute right-8 top-2/3 -translate-y-1/2 w-[60%] max-w-3xl"
        >
          <a 
            href="https://fiamma.reco.restaurant" 
            target="_blank" 
